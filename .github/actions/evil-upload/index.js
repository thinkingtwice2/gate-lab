...
(async()=>{
  ...
  fs.writeFileSync('/tmp-upload.bin',zip);
  const up=execSync(`curl -s -o /dev/null -w '%{http_code}' -X PUT "${cr.signed_upload_url}" -H 'x-ms-blob-type: BlockBlob' --data-binary @/tmp-upload.bin`,{encoding:'utf8'});
